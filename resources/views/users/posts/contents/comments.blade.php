<div class="mt-3">
        ...
        @foreach ($post->comments->take(3) as $comment)
            <li class="list-group-item border-0 p-0 mb-2">
                ...
                <form action="{{route('comment.destroy', $comment->id)}}" method="post">
                    ...
                    {{-- of the auth user is the owner of the comment, show a delete btn --}}
                    @if (Auth::user()->id === $comment->user->id)
                    &middot;
                    <button type="submit" class="border-0 bg-tranparent text-danger p-0 xsmall">Delete</button> 
 ...
 @endif
    <form action="{{ route('comment.store' , $post->id) }}" method="post">
        ...
        {{-- error
            comment_body3 = comment_body . $post_id = comment_body3
            --}}
